...
import { UserCredential, deleteUser, signInWithEmailAndPassword, signInWithPhoneNumber } from "@firebase/auth";
...
export class UserService{
   ...
   async setNewUserSettings(params:UserSettingsDto,token:string):Promise<UserSettingsDto>{
    let newSettings:UserSettingsDto=new UserSettingsDto();
    let userData:UserDataDto = await this.getUserData(token);
    const newUserData:UserDataDto = await this.updateUserData(userData,params);
   ...
   private async updateUserData(userData:UserDataDto,params:UserSettingsDto):Promise<UserDataDto>{
    userData.isAnonym=params.isAnonym;
    userData.name=params.name;
    userData.email=params.email;
    userData.phoneNumber=params.phoneNumber;
    const document = doc(collection(this.network.firestore,FirebaseColumns.USERS),userData.uid);
    await updateDoc(document,userData.toJson())
    return userData;
   }

   async changeProfilePhoto(params:ChangeProfilePhotoDto):Promise<ChangeProfilePhotoDto>{
    const imageRef:string=await this.network.setImageToStorage(params.imageAsByte,params.uid,"profilePhotos");
   ...
   private async setProfileImageToDb(uid:string,profileImage?:string){
    const document = doc(collection(this.network.firestore,FirebaseColumns.USERS),uid);
    await updateDoc(document,{'profileImage':profileImage});
   }

   async deleteProfileImage(params:UidReqDto):Promise<BooleanSingleResponseDto>{
   ...
   private async deleteUserPosts(uid:string){
    ...
    const postsCol = await collection(this.network.firestore,FirebaseColumns.POSTS);
   ...
   private async deleteUserFromAuthService(uid:string){
    ...
    const user:UserCredential=await signInWithEmailAndPassword(this.network.auth,userData.data()['email'],userData.data()['password']);
